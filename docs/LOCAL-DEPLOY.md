...
To serve `pm-content.json` locally, just enable the feature when installing the operator:
...
Apply the `charts/private-llm-pm-integration` chart separately inside your local KCP control plane (or via Flux) to create the `APIExport`, `ProviderMetadata`, and `ContentConfiguration` objects.
