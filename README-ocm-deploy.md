...
0) Create a fresh cluster (optional)
```bash
...
ocm add componentversions --create --file dist/ctf .ocm/component-constructor.yaml \
  VERSION="$VERSION" GITHUB_REPOSITORY_OWNER="$GH_OWNER" IMAGE_TAG="$SHA" CHART_TAG="$VERSION"
ocm transfer commontransportarchive dist/ctf "$OCM_REPOSITORY" --copy-resources --overwrite
```

4) Install controllers (kro + OCM + Flux)
```bash
...
6) Bootstrap the RGD and deploy
```bash
# Apply bootstrap without editing tracked files (requires envsubst)
...
# Create an instance to deploy the chart (also via envsubst)
